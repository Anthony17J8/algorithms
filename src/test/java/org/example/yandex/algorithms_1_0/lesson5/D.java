...
public class D {
    ...
    @Timeout(1)
    void test(int[] in, int r, long out) throws Exception {
        assertEquals(out, getResult(in, r));
    }

    private static Stream<Arguments> source() {
        return Stream.of(
                Arguments.of(new int[]{1, 3, 5, 8}, 4, 2),
                Arguments.of(new int[]{1, 4, 5, 6, 7}, 10, 0),
                Arguments.of(new int[]{1, 3, 5, 7, 9}, 1, 10)
        );
    }

    private static long getResult(int[] in, int r) {
        int left = 0;
        int right = 1;
        long res = 0;
        while (right < in.length) {
            if (in[right] - in[left] > r) {
                res = res + (in.length - right);
                left++;
            } else {
                right++;
            }
