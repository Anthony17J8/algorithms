...
public class G {
    ...
    public static void main(String[] args) throws Exception {
        fs.writeAll(getResult(fs.readStringAsIntArray()), '\n');
        fs.close();
    }
    ...
    private static Stream<Arguments> source() {
        return Stream.of(
                Arguments.of(new int[]{7, 3, 2, 1, 9, 5, 4, 6, 8, 0}, new int[]{2, 9}),
                Arguments.of(new int[]{2, 1, 0}, new int[]{2}),
                Arguments.of(new int[]{1, 2, 0}, new int[]{1}),
                Arguments.of(new int[]{2, 0}, new int[]{})
        );
    }
    ...
    private static int[] getResult(int[] in) {
        ...
        int[] res = new int[tree.currentIdx];
        Integer current = tree.root;
        dsf(tree, current, res);
    ...
    private static void dsf(Tree tree, Integer nodeIdx, int[] result) {
        if (nodeIdx == null) {
            return;
        }
        Integer[] n = tree.elements[nodeIdx];
        dsf(tree, n[2], result);
        if ((n[2] == null && n[3] != null) || n[2] != null && n[3] == null) {
            result[idx++] = n[1];
        }
        dsf(tree, n[3], result);
    }
