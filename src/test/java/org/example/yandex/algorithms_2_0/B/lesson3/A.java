...
public class A {
    ...
    public static void main(String[] args) throws Exception {
        int[] arr1 = fs.readStringAsIntArray();
        int[] arr2 = fs.readStringAsIntArray();
        fs.write(getResult(arr1, arr2));
        fs.close();
    }
    ...
    void test(int[] in1, int[] in2, int out) throws Exception {
        assertEquals(out, getResult(in1, in2));
    }


    private static Stream<Arguments> source() {
        return Stream.of(
                Arguments.of(new int[]{1, 3, 2}, new int[]{4, 3, 2}, 2),
                Arguments.of(new int[]{1, 2, 6, 4, 5, 7}, new int[]{10, 2, 3, 4, 8}, 2)
        );
    }

    private static long getResult(int[] in1, int[] in2) {
        Set<Integer> s = Arrays.stream(in1).boxed().collect(Collectors.toSet());
        return Arrays.stream(in2)
                .filter(s::contains)
                .count();
    }
