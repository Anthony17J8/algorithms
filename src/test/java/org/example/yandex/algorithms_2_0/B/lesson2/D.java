...
public class D {
    ...
    public static void main(String[] args) throws Exception {
        int[] LK = fs.readStringAsIntArray();
        int L = LK[0];
        int K = LK[1];
        int[] n = fs.readStringAsIntArray();
        fs.writeAll(getResult(n, L));
        fs.close();
    }
    ...
    @Timeout(1)
    void test(int[] in, int L, int[] out) throws Exception {
        assertArrayEquals(out, getResult(in, L));
    }


    private static Stream<Arguments> source() {
        return Stream.of(
                Arguments.of(new int[]{0, 2}, 5, new int[]{2}),
                Arguments.of(new int[]{1, 4, 8, 11}, 13, new int[]{4, 8}),
                Arguments.of(new int[]{1, 6, 8, 11, 12, 13}, 14, new int[]{6, 8})
        );
    }

    private static int[] getResult(int[] in, int l) {
        int[] result = new int[]{-1, -1};
        boolean isOdd = l % 2 == 1;
        if (isOdd) {
            return odd(in, l, result);
        } else {
            return even(in, l, result);
        }
    }

    private static int[] even(int[] in, int l, int[] result) {
        int midRight = l / 2;
    ...
    private static int[] odd(int[] in, int l, int[] result) {
