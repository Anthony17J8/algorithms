...
public class A {
    ...
    public static void main(String[] args) throws Exception {
        int[] NQ = fs.readStringAsIntArray();
        int n = NQ[0];
        long[] arr = fs.readStringAsLongArray();
        int q = NQ[1];
        long[] prfxsum = prfxsum(arr);
        while (q-- > 0) {
            int[] lr = fs.readStringAsIntArray();
            int l = lr[0] - 1;
            int r = lr[1];
            fs.write(getResult(prfxsum, l, r));
            fs.newLine();
        }
    ...
    private static long[] prfxsum(long[] arr) {
    ...
    private static long getResult(long[] prfxsum, int l, int r) {
        return prfxsum[r] - prfxsum[l];
    }
