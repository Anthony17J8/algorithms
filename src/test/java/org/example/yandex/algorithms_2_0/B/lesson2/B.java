...
public class B {
    ...
    @Timeout(1)
    void test(int[] arr, int out) throws Exception {
        assertEquals(out, getResult(arr));
    }
    ...
    private static Stream<Arguments> source() {
        return Stream.of(
                ...
                Arguments.of(new int[]{1, 0, 0, 0, 2}, 4),
                Arguments.of(new int[]{2, 0, 0, 0, 1}, 4),
                Arguments.of(new int[]{2, 0, 0, 2, 1}, 1),
                Arguments.of(new int[]{2, 2, 1, 2, 1}, 1),
                Arguments.of(new int[]{2, 1, 1, 1, 1}, 4),
                Arguments.of(new int[]{1, 1, 1, 1, 2}, 4)
        );
    }
    ...
    private static int getResult(int[] arr) {
        ...
        while (leftIdx < arr.length && rightIdx < arr.length) {
            l = leftIdx;
            while (rightIdx < arr.length && arr[rightIdx] != 2) {
                rightIdx++;
            }
            ...
            while (l < r) {
                if (arr[l] == 1) {
                    if (arr[leftIdx] != 2) {
                        result = Math.max(result, rightIdx - l);
                    } else if (arr[r] != 2) {
                        result = Math.max(result, l - leftIdx);
                    } else {
                        result = Math.max(result, Math.min(l - leftIdx, rightIdx - l));
                    }
                }
                l++;
            }
