...
public class С {
    ...
    private static Stream<Arguments> source() {
        return Stream.of(
                Arguments.of("a", 0),
                Arguments.of("ab", 1),
                Arguments.of("cognitive", 4),
                Arguments.of("abcdefgh", 4)
        );
    }
    ...
    private static int getResult(String word) {
        ...
        int[] idxs = getIndexes(word);
        return result(idxs[0], idxs[1], word);
    }

    private static int[] getIndexes(String word) {
        int[] ixs = new int[2];
        if (word.length() % 2 == 0) {
            int idxR = word.length() / 2;
            int idxL = idxR - 1;
            ixs[0] = idxL;
            ixs[1] = idxR;

        } else {
            int idxR = word.length() / 2 + 1;
            int idxL = word.length() / 2 - 1;
            ixs[0] = idxL;
            ixs[1] = idxR;
        }
        return ixs;
    ...
    private static int result(int idx1, int idx2, String word) {
        int counter = 0;
        int idxLeft = idx1;
        int idxRight = idx2;
        char[] charArray = word.toCharArray();
        while (idxLeft >= 0 && idxRight < charArray.length) {
            if (charArray[idxLeft] != charArray[idxRight]) {
                counter++;
            }
            idxLeft--;
            idxRight++;
