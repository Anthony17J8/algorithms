...
public class B {
    ...
    public static void main(String[] args) throws Exception {
        ...
        fs.write(getResult(in[0], in[1], in[2]));
        fs.close();
    }
    ...
    @Timeout(1)
    void test(int r, int i, int c, int out) throws Exception {
        assertEquals(out, getResult(r, i, c));
    }
    ...
    private static int getResult(int N, int i, int j) {
        return Math.min(N - Math.max(j,i) + Math.min(i,j), Math.abs(j - i)) - 1;
    }
