...
public class С {
    ...
    public static void main(String[] args) throws Exception {
        ...
        int[][] in = new int[N][2];
        while (idx-- > 0) {
            in[idx] = fs.readStringAsIntArray();
        }
    ...
    private static int getResult(int[][] in) {
        ...
        for (int[] ints : in) {
            ...
            int w = ints[1];
            chess[h][w] = 1;
            leftW = Math.min(leftW, w);
            rightW = Math.max(rightW, w);
            upH = Math.min(upH, h);
            downH = Math.max(downH, h);
        }
        int p = 2 * ((rightW - leftW + 1) + (downH - upH + 1));
        for (int i = upH; i <= downH; i++) {

            for (int j = leftW; j <= rightW; j++) {
                if (chess[i][j] == 0) {
