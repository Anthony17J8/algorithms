...
public class A {
    ...
    public static void main(String[] args) throws Exception {
        ...
        IntStream.rangeClosed(1, N).forEach(set::add);
        String command;
        while (!(command = fs.next()).equals("HELP")) {
            Set<Integer> s = Arrays.stream(command.split(" "))
                    .map(Integer::parseInt)
                    .collect(Collectors.toSet());
            if (contains(set, s) <= set.size() / 2) {
                fs.write(no);
                fs.newLine();
                set.removeAll(s);
            } else {
        ...
        fs.writeAll(set.stream().sorted(Comparator.naturalOrder()).collect(Collectors.toList()), ' ');
        fs.close();
    }

    private static int contains(Set<Integer> set1, Set<Integer> set2) {
        int cnt = 0;
        for (Integer i : set2) {
            if (set1.contains(i)) {
                cnt++;
