...
public class E {
    ...
    @Timeout(1)
    void test(String a, String b, String out) throws Exception {
        assertEquals(out, getResult(a, b));
    }
    ...
    private static Stream<Arguments> source() {
        return Stream.of(
                ...
                Arguments.of(
                        "abbaabaababb", "bbabbabaabab", "bbbbbb"
                ),
                Arguments.of(
                        "abbaaba", "bbabb", "bbb"
                )
    ...
    private static String getResult(String a, String b) {
        ...
        for (char c : a.toCharArray()) {
            mapA.compute(c, (k, v) -> v == null ? 1 : v + 1);
        }
        for (char c : b.toCharArray()) {
            mapB.compute(c, (k, v) -> v == null ? 1 : v + 1);
        }
        StringBuilder result = new StringBuilder();
        TreeSet<Map.Entry<Character, Integer>> s = new TreeSet<>(
                Map.Entry.<Character, Integer>comparingByKey().reversed().thenComparing(Map.Entry.comparingByValue()));
        while (idxa < a.length() && idxb < b.length()) {
            char froma = a.charAt(idxa);
            char fromb = b.charAt(idxb);
            if (mapA.containsKey(fromb) && mapB.containsKey(froma)) {
                if (froma < fromb) {
                    Integer iA = mapA.computeIfPresent(froma, (k, v) -> v - 1);
                    if (iA != null && iA == 0) {
                        mapA.remove(froma);
                    }
                    idxa++;
                } else if (froma > fromb) {
                    Integer iB = mapB.computeIfPresent(fromb, (k, v) -> v - 1);
                    if (iB != null && iB == 0) {
                        mapB.remove(fromb);
                    }
                    idxb++;
                } else {
                    s.add(Map.entry(froma, idxa + idxb));
                    idxa++;
                    idxb++;
                    Integer iA = mapA.computeIfPresent(froma, (k, v) -> v - 1);
                    if (iA != null && iA == 0) {
            ...
            } else if (!mapB.containsKey(froma)) {
                idxa++;
            } else if (!mapA.containsKey(fromb)) {
                idxb++;
            }
