...
public class D {
    ...
    @Timeout(1)
    void test(int[] in, long out) throws Exception {
    ...
    private static Stream<Arguments> source() {
        return Stream.of(
                Arguments.of(new int[]{1, 5, 2, 1}, 1),
                Arguments.of(new int[]{5, 12, 4, 3}, 24),
                Arguments.of(new int[]{10, 5}, 5),
                Arguments.of(new int[]{6, 2, 2}, 2),
                Arguments.of(new int[]{2, 2, 2, 2, 2, 10}, 20),
                Arguments.of(new int[]{5, 4}, 1)
        );
    }

    private static long getResult(int[] in) {
        ...
        for (int i = 1; i < in.length; i++) {
            ...
            m.compute(in[i], (k, v) -> v == null ? 1 : v + 1);
        }
        if (m.get(max) > 1 || max - (sum - max) < 1) {
            return sum;
        } else {
            return max - (sum - max);
        }
