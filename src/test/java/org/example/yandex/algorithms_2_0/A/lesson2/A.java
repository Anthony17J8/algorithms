...
public class A {
    ...
    public static void main(String[] args) throws Exception {
        long[] NK = fs.readStringAsLongArray();
        long N = NK[0];
        long K = NK[1];
        long[] in = fs.readStringAsLongArray();
        fs.write(getResult(in, K));
        fs.close();
    }
    ...
    @Timeout(1)
    void test(int k, long[] arr, int out) throws Exception {
        assertEquals(out, getResult(arr, k));
    }


    private static Stream<Arguments> source() {
        return Stream.of(
                Arguments.of(2, new long[]{45, 52, 47, 46}, 7)
        );
    }

    private static long getResult(long[] arr, long d) {
        long max = Math.max(arr[0], arr[1]);
        long min = Math.min(arr[1], arr[0]);
        int idx = 2;
